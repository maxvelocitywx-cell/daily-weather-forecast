    ...
    headlines: {
      ...
      items: {
        ...
        properties: {
          ...
          timestamp: {
            type: ['string', 'null'],
            description: 'ISO 8601 timestamp of the event (for real-time reports), or null if not applicable',
          },
          lat: {
            type: ['number', 'null'],
            description: 'Latitude of the event location (for map display), or null if not applicable',
          },
          lon: {
            type: ['number', 'null'],
            description: 'Longitude of the event location (for map display), or null if not applicable',
          },
        },
        required: ['headline', 'topic', 'regions', 'confidence', 'source_name', 'source_url', 'timestamp', 'lat', 'lon'],
